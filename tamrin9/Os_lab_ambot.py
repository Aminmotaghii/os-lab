...
GET_BORN_DATE, GET_EN_TEXT, GET_TEXT, GET_ID, ID_ERROR, KEYBOARD_ERORR = range(6)
...
def main():
    ...
    conv_handler0 = ConversationHandler(
        ...
        states={
            GET_BORN_DATE:
                [MessageHandler(~Filters.regex('^(/start|/age)$'), send_age),
                 CommandHandler('start', start),
                 CommandHandler('age', age)],
            GET_EN_TEXT:
                [MessageHandler(~Filters.regex('^(/start|/age)$'), send_voice),
                 CommandHandler('start', start),
                 CommandHandler('age', age)],
            MAX_ARRAY:
                [MessageHandler(~Filters.regex('^(/start|/age)$'), send_max),
                 CommandHandler('start', start),
                 CommandHandler('age', age)]},
